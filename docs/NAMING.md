...
A feature shares nothing with any other feature, with staging or production, in its own project or in another project in the same account (#643), except its rate-limit namespaces, which every feature shares and no declared environment can (below). The account and its one Secrets Store are the only containers that cannot be split; every name inside them is the feature's own. Two rules make that true of the names rather than likely:
...
**Rate-limit namespaces are the one thing features share, and only with each other.** Cloudflare keys a limiter's counters by `namespace_id` across the whole account. A feature binds an integer from 1000000000 through 1999999999, a range **no project running this kit may declare**, in any Worker or stanza: `pithy provision` for every environment and `pithy deploy` refuse one, and `pithy doctor` reports it, whether or not the project ever provisions a feature. The id is read as the integer it spells, so `"01031275746"` is `1031275746` and is refused too. So a feature never shares a limiter with staging or production.

Each limiter's feature namespace is fixed: its declared `namespace_id` plus 1000000000, so `1001` is `1000001001` in every feature of every project. An offset cannot collide, so two distinct limiters never merge, and two Workers bound to one namespace in production share one in every feature. A limiter whose namespace cannot be offset into the range is refused by `pithy provision --feature` before anything is created: one with no `namespace_id`, one not spelled in decimal digits, or one outside 1 through 999999999. Such an id is valid in staging and production; a feature needs one below 1000000000. Nothing is allocated, recorded or released, and nothing account-wide is created for rate limiting.
