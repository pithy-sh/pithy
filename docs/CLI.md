...
| `pithy feature` | Feature environment lifecycle: `create` (local worktree, ports, migrate + seed), `sync` (make an existing worktree ready), `provision` (its ephemeral CF resources, into a generated config under the already-ignored `.wrangler/` — a feature's ids are a build artifact and never touch a tracked file), `destroy` (tear it all down) |
