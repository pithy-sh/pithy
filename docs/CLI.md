...
| `pithy feature` | Feature environment lifecycle: `create` (local worktree, ports, migrate + seed), `sync` (make an existing worktree ready), `destroy` (tear it all down). All three derive the feature from the branch. `provision` survives as a deprecated alias that prints `pithy provision --feature` and runs it |
