...
| Flag | Meaning |
|---|---|
| `--env <env>` | Target environment: `dev`, `staging`, `prod`. **No default.** Omitted, each Worker's top-level `wrangler.jsonc` stanza ships, which is not an environment at all — and is refused while any binding in it has no id, which a project composing a capability always has. The kit's Workers do not ship either, because they are per environment. `dev` ships the Workers under `apps/` only: the kit's run locally under `pithy dev`. |
