...
- **An environment no sitekey can reach**: `board: live has no sitekey. Turnstile covers dev, staging, prod and a feature build, so sign-in there is blocked.` A **declared** environment beyond those names. The kit has no remedy to offer, so the line names none. A feature build is not reported: `TurnstileSitekeys` carries an optional `feature` key, and a branch build left with none resolves Cloudflare's always-pass test sitekey, so it renders. A `feature: ""` an adopter wrote is reported as a blank sitekey, like any other.
