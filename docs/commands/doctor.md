...
A **`Rate limiters:`** block appears when a Worker's tracked `wrangler.jsonc` declares a rate-limit `namespace_id` from 1000000000 through 1999999999, at the top level or in any stanza (#643). Pithy allocates every feature's namespaces from that range, for every project in the account, so a declared id there can be handed to some branch, and the two share counters. One line per id and where it is declared, then the remedy: a `namespace_id` below 1000000000. The id is read as the integer it spells, so `"01031275746"` is named as `1031275746`. It **fails the exit**: `pithy provision` and `pithy deploy` refuse the same stanza, whether or not the project has features. The kit writes limiters from 1000 through 9999, so no scaffolded project meets it.
