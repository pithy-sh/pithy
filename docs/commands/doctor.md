...
Ports:      ~/.config/pithy/dev-ports.json
            ...
            8847–8866  feature/9-billing  ← no worktree; pithy feature prune
...
Project capabilities:
  @pithy-sh/core         1.2.0 ✓
  @pithy-sh/auth         1.1.8 (1.2.0 available — run `pithy upgrade --packages`)
...
Beside it sits a **`capabilities:`** section, for a capability this project **composes** and has not **installed**. Those are two different questions, and a project can pass the first and fail the second: a Worker's `pithy.config.ts` is imported by path, so the composition loaded — and the package behind it is in neither the project root's `node_modules` nor that Worker's own. Nothing else in the report notices, because a capability with no manifest contributes no binding to check and no config option to read, so it reads as health right up until the next `pithy payments …` command refuses. Install a capability under `apps/api/node_modules` alone and it is **fine**: every `pithy` command looks along the project root's chain first and then in each Worker's own `node_modules`, which is what "capabilities are per Worker" invites and what a package manager that does not hoist produces on its own.
...
**It names what it checked, and the naming is the point.** The probe is two calls — verify the token, then list the account's permission groups — and both are Account → API Tokens. Nothing here touches Turnstile, D1, KV, R2, Vectorize, Workers AI, Email or Workers, so a token that holds none of those grants passes this line and then fails the first command that needs one. The line used to read `reachable (token active)`, which is a claim about products it had not asked about; it reads `token active; checked: API tokens — no other product was reached` instead. **Probing each product would be a worse claim, not a better one:** the per-product probe available is a *read* — Turnstile's lists widgets — so a token with Read and no Edit would pass every one of them and still be refused by `pithy turnstile provision`, under a line that had just said the product was checked. The set is not derivable either, since `@pithy-sh/turnstile` declares no bindings at all: it is provisioned over REST and consumed as a secret, so the one product this was reported about is exactly the one a binding-derived list omits. A grant a command actually needs is a capability's own account-tier check, in the [`Settings:`](#what-it-does) block below, which knows what the grant has to *do*.
...
The **`Ports:`** line names the file that decides every port `pithy dev` binds — `<config>/dev-ports.json`, one registry for the whole machine — and lists what is in it. **It used to live at the main repo root, and every project on a machine therefore kept its own.** Every one of those started empty and every one handed out the first block, so two projects on their default branch pinned the same twenty ports and the second refused to start with nothing able to say who had taken them. The registry is keyed by checkout now and spans every repository you have, which is what makes the guarantee machine-wide rather than per project — and is also why it needs this line: inside the checkout it was at least in the file tree you had open, and here nothing in your project mentions it. The path is reported whether or not the file exists yet; no file is the correct state until the first `pithy dev` or `pithy feature create`. If a `.dev-ports.json` from an older CLI is still sitting in your checkout, this line names it, because nothing reads it any more and editing it changes no ports. Delete it.

**The listing is the rest of the answer, and it is two questions rather than one.** This checkout's blocks come first and unqualified — which ports am I on, and on which branch. Everything under them is the machine-wide view, each block named by the checkout that holds it, which is what turns *8847, and I don't know why* into *because `other-app` holds 8827–8846*. Nothing new is recorded to support any of it: the registry is already `root → branch → block`, and every row above is read straight out of it.

**Ranges, never block indices.** A block's ports are `base + index × size`, so the index is the whole answer only while every entry is the same width — and a registry written before the block width changed keeps its old entries verbatim. Printing `8807–8826` is what makes a mixed-width registry legible, and it is the form the question arrives in: the port you are looking at is the one `pithy dev` just refused to bind.

**`← not on disk` is one of two lines here you can act on.** Pruning cannot tell a deleted checkout from a moved one — both are a missing directory — so a renamed or relocated repository has its blocks freed by the next allocation any project on this machine makes, and nothing anywhere reports that it happened. Today you find out when another project is handed those ports. This row is taken before the sweep, and it means: you renamed that directory.

**None of it can fail the exit**, the same rule the `Secrets:` and `Cloudflare:` lines are built on. Every line here reports a *location*, and a checkout that is gone from disk is information, not drift. A registry that will not parse says so in place of the listing, and names the file to delete — an empty listing would mean nothing holds any ports, which is the opposite of what a corrupt registry means, since `pithy dev` will not allocate against one at all.

**`← no worktree; pithy feature prune` is the other.** It marks one of this checkout's own blocks whose worktree directory is gone from disk and whose branch no longer exists locally — a feature removed some way other than `pithy feature destroy`, leaving only its block. A branch that still exists, or a directory still on disk, keeps a block unmarked. The sweep above never reaches these, because the checkout they are filed under is still on disk. [`pithy feature prune`](feature.md) frees it. The row is decided by the very function that command frees by, so every block marked here is one it frees and no other. It is only ever said of this checkout's own blocks, since a repository speaks only for its own branches, and it is never said where there is no repository to ask, nor in a bare repository's worktree or a submodule, where `prune` refuses.

There is deliberately no `pithy ports prune`. Pruning dead checkouts already runs under the lock on every allocation, including the idempotent one, so `pithy dev` on a settled branch already sweeps the machine. What was missing was never the sweeping — it was the report. A feature's block is the exception, and its command is `pithy feature prune`, which is not run for you: see [`feature.md`](feature.md).

In `--json` the same facts are `path`, `present`, `stray`, `root`, `unreadable`, `detail`, and `entries` — one object per block, carrying its `root`, `branch`, `block`, `base`, `size`, whether it is this checkout's (`own`), whether that root is still on disk (`onDisk`), and whether it is a block whose worktree directory and branch are both gone, which `pithy feature prune` frees (`orphaned`). Paths there are absolute and unabbreviated, like every other path in that payload.

`PITHY_CONFIG_DIR` moves the whole config directory — state file, `dev-ports.json`, `dev.json` and `secrets.jsonc` together. Set it in CI, which has no home directory worth writing to, and in any harness that must not touch your real values.
...
Ten things get a line, and every one of them names an absolute path rather than a convention:
...
- **A registry secret still copied into `<config>/<project>/dev.json` under `"vars"`.** `pithy seed` used to make that copy and the generator used to read it; the generator reads `secrets.jsonc` directly now, so nothing reads the copy. A value there that *no* registry declares — a machine-local endpoint — is a legitimate tenant of that file and says nothing. A `TURNSTILE_SITEKEY_*` there is not: nothing reads it, and the `Turnstile:` block below names it.
...
- **A key in the root `.dev.vars` that a Worker reads as a value** — a `secret` binding a capability requires, a `wrangler.jsonc` `vars` entry, or a name wrangler's `secrets.required` lists. Real value, wrong file, and never described as deletable.
- **A key in the root `.dev.vars` named like a binding, and read as a value by no Worker.** `EMAIL_SENDER is in .dev.vars, which no Worker reads. It is the name of board's workflows binding, and a value cannot stand in for a binding. Delete it.` A binding any Worker's `wrangler.jsonc` declares counts, in any stanza, read by the same reader as the `.dev.vars.local` lines below, and so does one a capability requires of any kind but `secret`. It is never sent to `dev.json`: that file reaches every Worker's generated `.dev.vars`.
...
- **A key nobody could classify**, because a Worker's config would not import. It is named, and nothing else is claimed about it. "Nothing reads it, delete it" is a *negative* claim, and the registry that would have settled it is exactly the one that could not be read — so that sentence is withheld until it can be made. The four verdicts above it survive a partial read, because each rests on positive evidence: a fixed credential list, a registry that did declare the name, a composition that does want or bind it. A store secret's binding is the exception, and is not called a binding: the registry nobody read may declare it as a secret.

The same block names what is in a **`.dev.vars.local`** — the root one, which reaches every Worker, and each Worker's own. It is for overrides, and three kinds of key get a line:

- **A key that shadows a secret.** `auth-session-secret in apps/board/.dev.vars.local shadows the secret of that name. Fine, and never silent.` That is what the file is for. A store secret counts under the binding it is read as, and a name wrangler's `secrets.required` lists counts too — wrangler dev loads it from `.dev.vars` as a secret, and it is never a variable.
- **A key named like a binding.** `EMAIL_SENDER in apps/board/.dev.vars.local has the name of board's workflows binding. wrangler dev ignores the value: the binding wins. Remove it.` Every binding the Worker's `wrangler.jsonc` declares counts, of every kind, at the top level, in every `env.<name>`, and in `previews`. What dev does with the value depends on where: wrangler dev keeps a top-level binding and drops a `.dev.vars` value of the same name, while `pithy dev` runs the top level, which has no binding declared only in an environment or in `previews` — so there the line says `in dev the Worker reads this string instead`. Copying the key into `vars` is never the answer: wrangler refuses a config where a var and a binding share a name.
- **A key nothing else declares.** `FEATURE_FLAG is in .dev.vars.local and nowhere else. If production needs it, declare it in wrangler.jsonc vars. If nothing reads it, delete it.` Doctor cannot know which. Withheld while a Worker's config will not import, like every negative claim in this block.

**Each Worker's own file is judged against that Worker.** Its own secrets come first, then its top-level bindings, then its `vars`, then bindings only an environment or `previews` declares. A sibling's registry secret comes last — every registry's secrets reach every generated `.dev.vars`, so overriding one is shadowing it — and so it never hides this Worker's binding. **The root file reaches every Worker, so it is judged against each.** Any Worker that binds the name makes it a binding line naming every such Worker. If another Worker reads the name as its own secret or a `vars` entry, the line says move the key into that Worker's `.dev.vars.local` instead of removing it: `API_URL in .dev.vars.local has the name of web's services binding. wrangler dev ignores the value: the binding wins. board reads it as a value: move it into apps/board/.dev.vars.local.`

None of the three fails the exit. It is in `--json` as `devVarsLocal`.
...
**A secret your configuration cannot reach is left out of all of it.** `auth` declares four OAuth credentials whichever providers you enabled; `support` declares the R2 credentials for a bucket you may have declined. Those are not *not yet done* — nothing here will ever read them — so they are not on the `No dev value for …` line and they are not in `Secret bindings:` either. One run used to say `SUPPORT_BUCKET (r2) declined in pithy.config.ts` under `bindings` and ask for `support-r2-credentials` a few lines later; the two halves read the same declaration now. The reason is still in `--json`, as `devSecrets.inapplicable`, so nothing disappears without saying why — and [`pithy secrets ls`](./secrets.md) lists every declared secret with the reason on the line, which is where *what would I change to turn this on* is answered. The master key is never filtered, whatever anything declares: it is the one gap that stops the local store opening at all.

**And *cannot reach* is asked of every environment, not of the run.** Your config is code, so a provider may be enabled for production alone. Doctor composes each Worker once per environment the root `pithy.config.ts` declares, plus `dev`, and a secret needed in any one of them stays on the `No dev value for …` line — which is why there is no `--env` here. [`pithy secrets ls`](./secrets.md) states the rule once, with the shape that makes it matter.

**And only a composition has an opinion.** An environment whose `pithy.config.ts` *throws* produced none — no capabilities, no registry, nothing that could need a secret — so it is left out of the union rather than counted as *everything applies*. That is a fault in its own right and it gets its own block, below.
...
An **`Environment inheritance:`** block appears when an `env.<name>` stanza silently goes without something its own top level declares. The block above asks whether the right stanzas exist; this asks what is inside one, and a project passes that one and fails this whenever a top-level key wrangler does not hand down was never repeated.

Most of a `wrangler.jsonc` flows down: declare `compatibility_date`, `observability` or `triggers` once at the top and every environment has it. A minority does not — `vars`, `version_metadata`, and every binding block among them. Those are taken from the stanza alone, so a key at the top level and absent from `env.staging` is **absent in staging**, whatever the top level says. There is no way to make one inherit: `env.<name>.vars` replaces the top-level block rather than merging it, and the same is true of every key on that list. **Repetition is the mechanism**, which is why the starter's own stanzas repeat what they repeat.

One line per Worker, environment and key, naming what that environment goes without: `api: version_metadata is at the top level and not in env.staging. Environments do not inherit it, so staging deploys without CF_VERSION_METADATA. Repeat it in env.staging.` The names come out of your own config rather than from a description of what the key is for, so the line says `CF_VERSION_METADATA` — the binding your code reads — instead of a paraphrase.

The kit's own first adopter shipped exactly that, on both deployed environments, and nothing said so for as long as nothing was deployed there. `verifyDeployedVersion` reads `CF_VERSION_METADATA` after a deploy and treats a Worker that answers without a version as inconclusive, so that check was permanently inconclusive on the only two environments where it matters.

**Which keys are not inherited is wrangler's answer, not the kit's opinion.** The kit states the list once, in `packages/cli/src/project/wranglerInheritance.ts`, and a test holds it to wrangler's own `notInheritable(…)` call sites and to its `EnvironmentNonInheritable` interface — so a wrangler release that adds a key, or moves one across the line, fails the build rather than quietly narrowing this check. That gate exists because the first hand-written version of this rule was wrong about half its names: `observability` and `triggers` read like per-environment settings and are both inherited.

**An empty collection at the top level is not a finding.** Wrangler warns on `"d1_databases": []` declared above and omitted below; doctor does not, because an empty array binds nothing and there is no cost to name. The moment one binding lands in it, every stanza that does not repeat it is reported.

It **reports and never fails the exit**, on the same rule `Dev secrets:` and `Secret bindings:` follow: every project scaffolded before this check landed is in this state for `version_metadata`, because the template it copied from was, and an upgrade that turns a green `pithy doctor` red in CI is a surprise rather than a diagnosis. It is in `--json` as `environmentInheritance`.

A **`Turnstile:`** block appears when a Worker's front end renders no Turnstile widget somewhere, or when a sitekey var nothing reads is still in a file.

**A sitekey is a build input.** The `pithy()` Vite plugin inlines the turnstile capability's client projection when the front end is built, and the projection reads `widgets.<mode>.sitekeys.<environment>` out of the Worker's `pithy.config.ts`. Where that is blank the bundle renders no widget — and `@pithy-sh/auth` still gates sign-in, so the gate fails closed and **nobody can sign in there**. Nothing else in the kit says so.

Three kinds of line, each with its own answer:

- **A blank sitekey**, per Worker that gates `login`: `board: no widget renders in staging and prod, so sign-in there is blocked. Run pithy turnstile provision --worker board, then redeploy.` Provisioning writes the sitekeys; the redeploy is what puts them in a bundle.
- **An environment no sitekey can reach**: `board: live has no sitekey. Turnstile covers dev, staging, prod and a feature build, so sign-in there is blocked.` A **declared** environment beyond those names. The kit has no remedy to offer, so the line names none. A feature build is not reported: `TurnstileSitekeys` carries an optional `feature` key, and a branch build left with none resolves Cloudflare's always-pass test sitekey, so it renders. A `feature: ""` an adopter wrote is reported, with its own remedy: `board: no widget renders in a feature build, so sign-in on a branch is blocked. Its sitekeys.feature is stated blank — remove that key and Cloudflare's test sitekey applies again, or state a real widget's. No pithy command writes it.` It is the one blank `pithy turnstile provision` does not fill, so the line does not name it.
...
An **`Environment configs:`** block appears when a declared environment's `pithy.config.ts` **throws while it is being composed**. The block above compares a stanza to a declaration and never evaluates either config; this one loads them. A config is code — `pithy init` scaffolds `originFor(compositionEnvironment(), DOMAINS)` into every one — so it can load under `dev` and refuse under `prod`, and a half-configured `prod` throwing *Billing is not configured for this environment* is the ordinary shape of it.

One line per environment, with the config's own action beneath it: `env.prod` · `pithy.config.ts throws when it is composed`, then `Set payments.billing in apps/api/pithy.config.ts, or drop prod from environments.`, then `Every command that composes prod fails the same way.` That last line is the size of the finding. `pithy deploy`, `pithy migrate`, `pithy provision` and `pithy dev` all evaluate that config, and every one of them fails exactly here.

It **fails the exit**, on the same standard the block above meets: the fault is established from the checkout's own files, no account is reached, nothing is inferred — and there is no day-one state to spare, because a freshly scaffolded project composes in every environment it declares. It is in `--json` as `environmentConfigs`.

**Doctor finds it because the secret checklist had to ask.** Deciding which secrets apply means composing each declared environment, which is the only thing in the CLI that takes all three compositions — so it is the first thing in a run that learns `prod` will not load. It used to feed that failure back into its own answer as *everything applies*, which quietly switched off the marking described above wherever one environment was half-configured. It contributes nothing now, and what it found is reported here instead.

A **`Secret bindings:`** block appears when a declared environment's `wrangler.jsonc` stanza does not bind a `cf-secrets-store` secret that Worker reads. It is the deployed half of the block above: `Dev secrets:` is about the machine-local file, this is about the stanza a Worker boots against in staging or prod. One line per Worker and environment — `board env.staging binds no CONNECTION_KEY_ENCRYPTION_KEY, SECRETS_ENCRYPTION_KEYS.` — because one command answers all of them. A line names the **binding**, which is the secret's registry key in SCREAMING_SNAKE_CASE: `email-link-signing-key` is bound, and read, as `EMAIL_LINK_SIGNING_KEY`. A stanza binding it under the key itself binds nothing the Worker reads, and is reported like any other gap.
...
A **`Settings:`** block answers the question every other check here leaves open: do the settings **work**, or are they merely present. Config drift, bindings, migrations, names, origins — all of them ask whether something is *there*. None of them looks at the value. So a project stays entirely green while `fromAddress` names a domain nobody onboarded, the link-signing key was never created, `BASE_URL` points at a host nothing serves, `EMAIL_THEME` does not parse, and the suppression database does not exist. Nothing arrives, and nothing says why.

**A capability declares its own check, on its `Capability` object.** Discovery keys on the composed instance and never on `pithy.manifest.json` — a manifest is a file a capability package can be shipped without, as `@pithy-sh/matchmaking` and `@pithy-sh/rating` were from the day they landed until #415, and a manifest-keyed rule skips such a package without a word. Doctor runs the check for every capability every Worker under `apps/` composes, with no capability named on the command line, and `--worker <name>` narrows it exactly as it narrows the health block. **A capability that declares nothing is skipped in silence, and that is not a fault** — the block does not appear at all when nothing in the project declares a check.

There are **two tiers**, because they cost different things:

- **local** — does the value parse, is it the right shape, is it right for this environment. Free, offline, always run. It validates through **the same Zod object the capability's host Worker validates at boot**, so a value doctor calls good is a value the host will accept, and a rule that changes changes in one file.
- **account** — is the sending domain a zone here, does the secret have a value, is the database there. One Cloudflare call, on the credentials this report already resolved. No second credential path.

**Both are faults.** A local finding fails the exit. An account finding fails it too — mail that will not send is not a warning — **but only when the account was actually reached**. When it cannot be, the tier is reported as skipped with the reason, exactly as the version checks report `version check skipped (offline)`, and it gates nothing: a check nobody ran established nothing.

```
Settings:
  api:
    email        BASE_URL — Links are built against http://localhost:8787, and no environment this project declares answers on it.
                 Set `email({ baseUrl })` to an origin this project serves: https://api.acme.dev.
    email        fromAddress — acme.dev is not a zone on this Cloudflare account, so it cannot be onboarded onto Email Service.
                 Add acme.dev to this Cloudflare account, then onboard it onto Email Service in the dashboard.
```

The block reports one state at a time. An account finding is only ever reported when the account answered, so a run that skipped the tier carries no account line among the findings — it carries the reason instead, indented under the Workers: `account checks skipped (offline) — nothing here was established about the account.`

Every finding is a **problem line and an action line**, naming the `pithy` command, the `pithy.config.ts` key, or the one-time account action that resolves it. A capability whose check threw is named with the tier that failed — `local checks couldn't be run` — never folded into the findings and never counted as a pass. When the probe never got as far as a capability at all — the root config states no name, a Worker's `pithy.config.ts` will not import — the block says `the checks could not be run, so nothing here was established` and blames no account, because nothing here reached for one. A clean run collapses to one line, `Settings: every composed capability's settings work ✓`, and that line prints in the terse report too: this is the one block where silence would be read as an answer it has not established.
...
The verdict is **the same one `pithy dev` reaches** — literally the same preflight, worded once — so the two commands cannot disagree about one project. Which capability answers is the host registry's business: `email` holds the kit's only binding that sends a message, and a tenth capability that puts something on the wire is reported here with no change. **It is not a fault and never fails the exit** — `email({ devDelivery: "simulator" })` selects the simulator deliberately, and an offline machine has no other option — but it prints in the terse report too, because silence would be read as "of course it sends". The block is absent when nothing composed sends anything.

A **`Shared runtimes:`** block appears when this project resolves more than one copy of `zod`, `kysely` or `hono`. Every kit package declares those three as `peerDependencies`, so an installer puts one at the top where your own `import { z } from "zod"` finds the same copy the kit uses. A peer is a request rather than a guarantee, though: a range conflict, a `--legacy-peer-deps`, a pinned resolution or a workspace hoisting differently all reintroduce a second copy.

```
Shared runtimes:
  More than one copy of kysely (2). Two copies are two types, and the compiler will not say so.
    kysely: /Users/jo/code/acme/node_modules/kysely
    kysely: /Users/jo/code/acme/node_modules/@pithy-sh/auth/node_modules/kysely
  Declare each one in your own package.json, at one range, and reinstall.
```
...
A **`Self binding:`** block appears when a project whose root `pithy.config.ts` declares `administersItself: true` has an environment stanza that binds no `SELF`. **A Worker cannot fetch its own hostname**: the subrequest loops back through the edge into the Worker it came from and hangs until Cloudflare answers 522, on a route that answers in a second and a half from outside. So a deployment that points a management client at itself dispatches through the runtime instead — `env.SELF.fetch(request)` — and `pithy provision` writes the binding into every stanza it generates, named for that environment's own script. One line per Worker and environment, then the command that writes them. It **fails the exit**: the contradiction is between the project's own two files, and the symptom otherwise is a timeout in production that every dashboard between here and the browser attributes to Cloudflare. A project that declares nothing gets no block and no line — nothing is inferred from what a project composes, because composing the control plane is what makes a Worker administrable by anybody. **A binding is its target, not its name**: an entry called `SELF` naming a script the stanza does not deploy as is reported the same way and says both names, because that is the fault a `pithy worker rename` or a hand-edited `env.<name>.name` leaves behind, and it provisions clean while refusing at runtime. An environment with no stanza at all is `Environments:`' finding, not this one; a feature environment is nobody's, since its stanza is generated on every run. **`dev` is not reported**: the top-level stanza carries no binding on purpose, because a Worker reaching its own address locally is an ordinary request that returns — the loop that ends in a 522 needs the edge.

A **`Rate limiters:`** block appears when a Worker's tracked `wrangler.jsonc` declares a rate-limit `namespace_id` from 1000000000 through 1999999999, at the top level or in any stanza (#643). Every feature of every project in the account binds its limiters in that range, so a declared id there shares counters with a branch. One line per id and where it is declared, then the remedy: a `namespace_id` below 1000000000. The id is read as the integer it spells, so `"01031275746"` is named as `1031275746`. It **fails the exit**: `pithy provision` and `pithy deploy` refuse the same stanza, whether or not the project has features. The kit writes limiters from 1000 through 9999, so no scaffolded project meets it.

An **`AI clients:`** block appears when an AI coding client installed on this machine could be reading the Pithy documentation and is not (#652). `pithy docs connect` writes an MCP entry for `https://pithy.sh/mcp` into whichever of the ten clients it knows about are installed here; this block is what says one of them never got it. One line per such client, naming the tool and the command that connects it — `Cursor — pithy docs connect --client cursor` — then the remedy beneath them: ``Each line is one command. `pithy docs status` shows where each one would land.`` A client is named only when it is **detected here** and **no configuration of it carries an entry**, so a machine that runs none of the ten says nothing, and neither does one where every client already reads the docs.

Files, and only files that are already there: nothing is created, nothing is written, and a config that will not open is simply not counted as connected. A diagnostic that repaired things would be the last thing an operator with a broken environment needs.

It **reports and never fails the exit**, on the same rule the `Alias:` line follows: whether an editor is wired to a documentation server is the operator's own business, and a health check that went red over a declined offer would be telling somebody their machine is misconfigured for saying no. It is not a term in the terse predicate either, so it never drags the rest of the report verbose — it is short, it is actionable, and it prints in both forms. `pithy docs status` (`docs/commands/docs.md`) is the whole picture, per client and per scope; this block is the part of it worth interrupting a health check for. It is in `--json` as `docsMcp`.
...
The same block carries two findings about the name a Worker deploys under **in each environment** (`env.<name>.name`).

**A stanza naming a capability's own host Worker is a fault, and it fails the exit.** A Worker deploys as `<project>-<env>-<worker>`, and a capability's host Worker deploys as `<project>-<env>-<capability>` — so `apps/email` in a project called `acme` would deploy `acme-staging-email`, byte-identical to what `pithy email provision` created. Cloudflare's script namespace is account-flat: `wrangler deploy --env staging` replaces it, the capability's Workflows stop running, and nothing anywhere says so. The reserved set is read from the host registry at run time — it is every capability that ships a host Worker, not a list written down here — and `pithy init --worker`, `pithy worker add` and `pithy worker rename` all refuse such a name outright. This line is for a project that already has one. **It needs no top-level `name` to run.** A `wrangler.jsonc` where every stanza names itself and the top level names nothing is legal, it is the shape this repo's own dashboard prod stanza takes, and the check used to skip such a config whole — so the one finding that stops an adopter silently replacing a capability's Worker never ran on it. Only the suffix note below ever needed that name.

**A Worker still on wrangler's suffix is reported and nothing more.** With no `env.<name>.name` wrangler appends the environment to the top-level name — `acme-api-staging` rather than `acme-staging-api` — which is what every project scaffolded before this convention carries. The line names where the Worker lands today and where the convention would put it, and says that moving is optional. It is: a Worker's name is the adopter's, and a rename means a new script, a route moved onto it, and a window where the old name is still answering. It **never fails the exit** and it never drags the rest of the report verbose.

**And it is only written where the convention names something you are allowed to create.** For an `apps/email` the convention composes `acme-staging-email` — the email capability's own host Worker, and the exact name the paragraph above fails the exit over. So the note recommended, as a friendly optional suggestion, the collision the other half exists to find, and an adopter who took the advice created it. A Worker there is no legal convention name for gets no note at all: its suffixed name collides with nothing, so there is nothing to say.
...
| `workerNames` | object \| null | `state`, one `mismatches` entry per stamp that disagrees, one `reserved` entry per environment stanza naming a capability's own host Worker (`worker`, `env`, `name`, `capability`), and one `convention` entry per Worker still on wrangler's environment suffix (`worker`, and an `environments` list of `{env, current, convention}`). Each entry in all three carries its own `detail`. `reserved` is part of `state: "drifted"` and fails the exit; `convention` never reaches `state` and never does. |
| `selfBinding` | object \| null | `state` (`ok`, `could-not-check`, `unbound`), `declared` — whether the root `pithy.config.ts` says `administersItself` at all, so an empty `missing` can be told from a project that never asked — and one `missing` entry per Worker-and-environment whose stanza binds no `SELF`, each carrying its own `detail`. `unbound` fails the exit. |
| `featureRatelimitIds` | object \| null | `state` (`ok`, `could-not-check`, `reserved`) and one `findings` sentence per declared rate-limit `namespace_id` in the feature range, 1000000000 through 1999999999, naming the Worker, the id as its integer, and the stanza. `reserved` fails the exit. |
| `docsMcp` | object \| null | `state` (`ok`, `unconnected`, `could-not-check`), one `findings` line per AI client detected on this machine that no configuration connects — each naming the tool and the `pithy docs connect --client <id>` that connects it — and a `detail` sentence counting them. A machine running none of the ten clients is `ok`, because nothing is wrong with it. Never gates the exit. |
...
| `environmentInheritance` | object \| null | `state` (`ok`, `could-not-check`, `unrepeated`), and one `unrepeated` entry per Worker-and-environment-and-`key` whose stanza does not repeat a non-inherited top-level key — `carries` names the bindings or variables that environment goes without, read out of your own top-level value, and each entry carries its own `detail`. Never gates the exit. |
| `turnstileSitekeys` | object \| null | `state` (`ok`, `could-not-check`, `findings`); `unrendered`, one entry per Worker and environment whose build renders no Turnstile widget — `slot` is `true` for a blank sitekey `pithy turnstile provision` fills, `false` for an environment no sitekey can reach; `stranded`, one entry per `TURNSTILE_SITEKEY_*` var nothing reads, with its `worker` (`null` for `dev.json` and the root `.dev.vars`), `environment`, absolute `file`, and `removedBy`, the Worker whose `pithy turnstile provision --worker` clears it or `null` when no command does; and `detail`, the report's lines. Never gates the exit. |
...
| `secretBindings` | object \| null | `state` (`ok`, `could-not-check`, `unbound`), one `missing` entry per Worker-and-environment-and-`binding` that a declared environment does not bind — each naming the `secret` (its registry key, which `pithy secrets create` takes) beside the `binding` it is read through — and a `detail` line per Worker-and-environment. `null` when no Worker composes `secrets`. |
| `settings` | object \| null | `state` (`ok`, `faults`, `could-not-check`), `account` (`state` is `checked` or `skipped`, with the `reason` — `offline`, `no-credentials`, `unreachable`, `not-declared`), `checked` (worker-and-capability pairs whose check ran), one `findings` entry per setting that does not work — each with its `tier`, `setting`, `environment`, `problem`, `action` and its own `detail` — `unchecked` (worker-and-capability-and-`tier` nobody could answer for), and a `detail` for the account tier. `null` when no composed capability declares a check — and never that when the probe itself could not run, which reports `"state":"could-not-check"` with both lists empty. |
| `localDelivery` | object \| null | `live` — whether a message sent from this machine leaves it — the `capability` that answers for delivery, and a `detail` carrying the run's own sentence about why. `null` when nothing composed sends anything. Never gates the exit: the simulator is a choice. |
| `sharedRuntimes` | object \| null | `duplicated` names each of `zod`, `kysely` and `hono` this project resolves more than one copy of — empty is healthy — `copies` maps each to every directory something resolved it from, and `detail` carries the sentence. `null` outside a readable project. Never gates the exit. |
| `devVarsLocal` | object \| null | Every `.dev.vars.local` key, root and per Worker, classified, with its `detail`. Each finding is `{ key, file }`. `shadowing`: a registry secret, overridden locally. `shadowingBinding`: a key named like a binding, with `bindings` — each Worker that binds it, the binding's `kind` (`workflows`, `d1_databases`, …) and `in`, its stanza (`null` for the top level, else `env.<name>`, `previews` or `env.<name>.previews`) — and `moveTo`, the files of the Workers that read it as a value. `devOnly`: a key nothing else declares. `unresolvable`: Workers whose config would not import. Never gates the exit. |
...
| A capability setting that does not work — from either tier | Exit 1. The account tier contributes findings only when the account was reached, so a run that skipped it cannot fail here. |
