...
**A stanza naming a capability's own host Worker is a fault, and it fails the exit.** A Worker deploys as `<project>-<env>-<worker>`, and a capability's host Worker deploys as `<project>-<env>-<capability>` — so `apps/email` in a project called `acme` would deploy `acme-staging-email`, byte-identical to what `pithy email provision` created. Cloudflare's script namespace is account-flat: `wrangler deploy --env staging` replaces it, the capability's Workflows stop running, and nothing anywhere says so. The reserved set is read from the host registry at run time — it is every capability that ships a host Worker, not a list written down here — and `pithy init --worker`, `pithy worker add` and `pithy worker rename` all refuse such a name outright. This line is for a project that already has one.
