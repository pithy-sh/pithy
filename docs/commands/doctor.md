...
An **`Origins:`** block appears when an environment answers on an origin its own configuration does not name. Every auth `baseURL`, OAuth callback, magic-link URL and CSRF allowed-origin is derived from that answer, so when it is missing each of them invents its own — and the dangerous invention is production's origin, which is how a staging deploy emails real users magic links into production. Two faults:
...
| `origins` | object \| null | `state`, and one `drift` entry per Worker-and-environment serving an origin its config does not name — `fault` is `no-origin` or `workers-dev-open`, `origin` is the one the config does name (`null` when it names none), and each entry carries its own `detail`. |
...
| `devSecrets` | object \| null | The dev-secrets findings, `mode` octal-formatted, `unresolvable` naming the Workers nobody could read. |
