...
There is deliberately no `pithy ports prune`. Pruning already runs under the lock on every allocation, including the idempotent one, so `pithy dev` on a settled branch already sweeps the machine. What was missing was never the sweeping — it was the report.

In `--json` the same facts are `path`, `present`, `stray`, `root`, `unreadable`, `detail`, and `entries` — one object per block, carrying its `root`, `branch`, `block`, `base`, `size`, whether it is this checkout's (`own`), and whether that root is still on disk (`onDisk`). Paths there are absolute and unabbreviated, like every other path in that payload.
