...
- **A key that shadows a registry secret.** `auth-session-secret in apps/board/.dev.vars.local shadows the secret of that name. Fine, and never silent.` That is what the file is for. A store secret counts under the binding it is read as, too.
- **A key named like a binding.** `EMAIL_SENDER in apps/board/.dev.vars.local shadows the workflows binding of that name. In dev the Worker reads a string where the binding should be. Remove it.` Every binding the Worker's `wrangler.jsonc` declares counts, at the top level and in every `env.<name>`, of every kind; the root file is judged against every Worker's. Copying the key into `vars` would shadow the binding in production too.
...
| `devVarsLocal` | object \| null | Every `.dev.vars.local` key, root and per Worker, classified, with its `detail`. Each finding is `{ key, file }`. `shadowing`: a registry secret, overridden locally. `shadowingBinding`: a key named like a binding the Worker's `wrangler.jsonc` declares, with the binding's `kind` (`workflows`, `d1_databases`, …). `devOnly`: a key nothing else declares. `unresolvable`: Workers whose config would not import. Never gates the exit. |
