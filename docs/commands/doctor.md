...
**`← no worktree; pithy feature prune` is the other.** It marks one of this checkout's own blocks whose branch no worktree of the repository has checked out — a feature whose worktree was removed by `git worktree prune`, a teardown script, or by hand, rather than by `pithy feature destroy`. The branch outlived the worktree and the block outlived both, and the sweep above never reaches it, because the checkout it is filed under is still on disk. [`pithy feature prune`](feature.md) frees it. The row is decided by the very function that command frees by, so every block marked here is one it frees and no other. It is only ever said of this checkout's own blocks, since a worktree listing speaks for one repository, and it is never said where there is no repository to list.
...
In `--json` the same facts are `path`, `present`, `stray`, `root`, `unreadable`, `detail`, and `entries` — one object per block, carrying its `root`, `branch`, `block`, `base`, `size`, whether it is this checkout's (`own`), whether that root is still on disk (`onDisk`), and whether it is a feature block no worktree holds, which `pithy feature prune` frees (`orphaned`). Paths there are absolute and unabbreviated, like every other path in that payload.
