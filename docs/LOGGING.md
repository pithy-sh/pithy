...
`version` is the Cloudflare version id, from the `version_metadata` binding the scaffold declares as `CF_VERSION_METADATA`. It is what answers the first question anyone asks when a deploy goes wrong: which build produced this line? The binding is declared at the top level of `wrangler.jsonc` **and repeated in every `env.<name>` stanza**, because wrangler does not inherit `version_metadata` into an environment: a stanza that omits it deploys with no binding at all, and every record from that environment is missing its `version`. `pithy init` and `pithy worker add` write it into each stanza, `pithy upgrade` adds the top-level declaration to a project scaffolded before it existed, and `pithy doctor` reports a stanza that does not repeat it. A Worker that does not declare it still logs — the field is simply absent, which reads as "cannot say" rather than as a build to trust.
