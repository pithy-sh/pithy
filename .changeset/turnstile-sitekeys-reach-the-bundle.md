...
- **Stranded vars are removed.** Provision and deprovision strip any `TURNSTILE_SITEKEY_*` from the Worker's `wrangler.jsonc` and from `dev.json`. `pithy doctor` names any still in a file.
