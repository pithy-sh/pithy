...
The app supplies screen, build, platform, environment, and locale, and that set is closed: an undeclared key is refused rather than stored, so this cannot become a telemetry pipe pointed at a support inbox. Attachment bounds are stated for a direct upload rather than inherited — 5 MB, three files, and a content-type allowlist, where the mail path has no type restriction at all and would have shipped a 10 MB any-type upload endpoint by default. Bytes arrive base64, either alphabet, and the size bound is measured on the decoded length; the declared filename is sanitised by the same rule the mail path uses, so a right-to-left override cannot make an executable render as a screenshot in a console. Ten submissions per account per hour, counted over the same sliding window the mail guard uses and **only against its own channel**, so neither surface can starve the other.
