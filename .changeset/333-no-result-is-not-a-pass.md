...
`turbo` cancels every remaining task when one fails. With a package that fails intermittently, that turns a run into a trap: the output names the familiar flake and says nothing about the packages that were cancelled behind it. During the Paddle wave a run reported only the known `@pithy-sh/storage` timeout while `@pithy-sh/payments:test` — 6,415 insertions of that wave's work — never started. Reproduced before changing anything: one planted failing test made `bun run test` print `Tasks: 0 successful, 22 total` with a single package named. Twenty-one produced no result and none of them was named.
