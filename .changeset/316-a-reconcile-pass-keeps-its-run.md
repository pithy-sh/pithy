...
That matters more here than it would elsewhere. **Reconciliation is the compensating control for a delivery mechanism that is known to fail.** When a webhook goes missing, an adopter's entitlement state silently diverges from what a customer paid: somebody paid and is not unlocked, or cancelled and still is. The pass that repaired it is the proof that happened, and it was the least observable thing the capability produced.
