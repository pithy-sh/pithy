...
The provisioner was never the missing part. `pithy feature provision` already resolves the Worker set, creates one resource per binding name, writes the ids into each Worker's own config and migrates. What was missing was the command, and the naming.
...
That closes a hazard that was reachable before it: `pithy feature provision --env staging` composed `<project>-f<issue>-<slug>-db` and wrote it in as staging's `DB`, in a checked-in `wrangler.jsonc`, then ran a remote migrate against it. Nothing refused it. **`pithy feature provision` no longer takes an `--env`** — the combination cannot be expressed, rather than being on a list of things not to do.
