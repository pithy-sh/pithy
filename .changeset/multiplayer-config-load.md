...
The invariant is a gate, not a fixed bug: `configEntrypoints.test.ts` imports every catalogued capability's entry point in its own Node process and requires it to resolve. It names no forbidden module — it performs the import — so the next entry point that reaches for a runtime-only one fails in CI rather than in an adopter's CLI.
