...
The refresh is chunked — 32 rows per statement, walked by keyset — because D1 runs one query at a time and caps a query at 30 seconds. An unbounded rewrite would hold the only thread against live submissions. It runs as a **cron-triggered Cloudflare Workflow** that checkpoints the keyset cursor per batch, so a board of any size ranks across as many durable steps as it needs and resumes from the last checkpoint on a crash — there is no per-run entry ceiling.
