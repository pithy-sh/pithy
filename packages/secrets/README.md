...
- **`devValue`** — optional. Set it to `random` when the value is *arbitrary*, and `pithy add <capability>` mints this project's dev value into `.dev.vars` (see below). Leave it off when the value must match something outside the project.
