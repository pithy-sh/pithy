...
describe("mintDevValue", () => {
  ...
  test("carries 256 bits of entropy, url-safe and unpadded", () => {
    ...
    const value = mintDevValue("random");
    expect(value).toMatch(/^[A-Za-z0-9_-]{43}$/);
  });

  test("covers every declared kind, so a new one cannot ship unminted", () => {
    for (const kind of DevSecretValue.options) expect(mintDevValue(kind).length).toBeGreaterThan(0);
  });
});
