...
import { readSecretRotations, readSecretStatus, type SecretsStatusDb } from "./status";
...
describe("readSecretStatus", () => {
  test("reports one row per named registry entry, and never a keyed one", async () => {
    const statuses = await readSecretStatus(db(), registry, { now: NOW });
  ...
  test("never rotated is null, which is not the same fact as rotated long ago", async () => {
    ...
    const byName = new Map((await readSecretStatus(db(), registry, { now: NOW })).map((s) => [s.name, s]));
  ...
  test("a failed rotation never advances freshness", async () => {
    ...
    const status = (await readSecretStatus(db(), registry, { now: NOW })).find((s) => s.name === "auth-signing-key");
  ...
  test("a Secrets Store secret has no row here, and its backend is what makes the nulls readable", async () => {
    const master = (await readSecretStatus(db(), registry, { now: NOW })).find(
  ...
  test("reads more names than D1 will bind in one statement", async () => {
    ...
    const statuses = await readSecretStatus(db(), defineSecretRegistry(many), { now: NOW });
...
describe("readSecretRotations", () => {
  test("returns the attempts newest first, in metadata only", async () => {
    ...
    const rotations = await readSecretRotations(db(), "auth-signing-key", 25);
  ...
  test("honours the cap, keeping the newest", async () => {
    ...
    const rotations = await readSecretRotations(db(), "auth-signing-key", 3);
  ...
  test("attempts recorded in the same millisecond keep a stable order", async () => {
    ...
    const first = await readSecretRotations(db(), "auth-signing-key", 2);
    const second = await readSecretRotations(db(), "auth-signing-key", 2);
