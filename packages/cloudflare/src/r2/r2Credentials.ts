...
export const R2Credentials = z
  .object({
    accessKeyId: z.string().min(1).meta({ multiline: false }).describe("The R2 S3 access key id."),
    secretAccessKey: z
      .string()
      .min(1)
      .meta({ multiline: false })
      .describe("The R2 S3 secret access key. Never logged or serialized to clients."),
  })
