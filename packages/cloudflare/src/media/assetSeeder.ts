...
/**
 * Seed-time byte uploads to the shared Cloudflare Images and Stream stores. Seeding hands a manager
 * raw bytes and a metadata bag and gets back the minted asset id/uid to record — the one place these
 * two stores accept fixture bytes from a CLI/CI context. Every wire response is validated with a
 * local Zod object so a shape drift fails loudly (`cloudflare/invalid_response`) instead of surfacing
 * as `undefined`. These calls are always remote: neither store has a local emulation.
 */
...
/**
 * Upload image bytes to Cloudflare Images with the given metadata, returning the minted id. Images
 * supports a direct byte upload, so this is a single `uploadImage({ file, metadata })` call — the
 * manager JSON-encodes the metadata bag for the wire.
 */
export async function uploadImageBytes(
  ...
  const response = await manager.uploadImage({ file, metadata: opts.metadata });
 ...
 * method, so this mints a one-time upload URL with `createDirectUpload` — carrying the metadata as
 * `meta` so it lands with the video — then POSTs the bytes to that URL as `multipart/form-data` (the
...
export async function uploadStreamBytes(
  ...
  bytes: Uint8Array | Blob,
  opts: { metadata?: Record<string, string>; maxDurationSeconds?: number } = {},
): Promise<{ uid: string }> {
