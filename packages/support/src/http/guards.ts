 ...
 *
 * **Every route here is `control-plane` and there are no others.** A support inbox has no end-user
 * surface: a customer interacts with it by sending email, not by calling an API, so there is nothing
 * for `requireAuth()` to gate and adding it would be adding a door to a wall. The seam's gate is
 * imported from core rather than copied — core is a hard dependency of every capability, so importing
 * its gate cannot leave a deployment without one, and with the seam uncomposed
 * `requireControlPlane` raises `controlplane/not_connected` rather than passing. Default-denied,
 * which for an inbox holding other people's private correspondence is the only acceptable default.
 *
