...
// imports `cloudflare:workers` (Workers-runtime only), so it can't be eagerly imported here — exclude it.
...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 52 modules, 72 schemas and 473 fields on
    ...
    expect(walk.modules).toBeGreaterThanOrEqual(49);
    expect(walk.schemas).toBeGreaterThanOrEqual(68);
    expect(walk.fields).toBeGreaterThanOrEqual(449);
  });
