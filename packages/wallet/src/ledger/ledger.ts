...
import { WALLET_ACCOUNTS_TABLE, WALLET_HOLDS_TABLE, WALLET_TRANSACTIONS_TABLE, walletDatabase } from "../data/tables";
import { WalletTransaction } from "../data/transaction";
import {
  WalletHoldNotFoundError,
  WalletHoldNotOpenError,
  WalletInsufficientFundsError,
  WalletInvalidAmountError,
} from "../error/errors";

/**
 * The wallet ledger — every balance movement, made correct by the database rather than by hope.
 *
 ...
 *   `wallet/insufficient_funds`. The guard is in SQLite, so no race can slip past it.
 *
...
export interface Ledger {
  ...
  transactions(userId: string, currency: string, limit: number): Promise<WalletTransaction[]>;
}
...
function assertAmount(amount: number, context: string): void {
  if (!Number.isInteger(amount) || amount <= 0) {
    throw new WalletInvalidAmountError({ detail: `${context}: amount ${amount} is not a positive integer.` });
  }
}
...
export function ledger(d1: D1Database, now: () => number = () => Date.now()): Ledger {
  const db = walletDatabase(d1);
  ...
  const ensureAccount = (userId: string, currency: string, at: number) =>
    db
      .insertInto(WALLET_ACCOUNTS_TABLE)
  ...
  const entry = (
    ...
    db
      .insertInto(WALLET_TRANSACTIONS_TABLE)
  ...
  const adjust = (userId: string, currency: string, set: Record<string, unknown>, at: number) =>
    db
      .updateTable(WALLET_ACCOUNTS_TABLE)
  ...
  const readBalance = async (userId: string, currency: string): Promise<Balance> => {
    const row = await db
      .selectFrom(WALLET_ACCOUNTS_TABLE)
      .select(["balance", "held"])
      .where("userId", "=", userId)
      .where("currency", "=", currency)
      .executeTakeFirst();
  ...
  const applyFunding = async (
    ...
    try {
      ...
      if (isViolation(error, "CHECK"))
        throw new WalletInsufficientFundsError({ detail: insufficientDetail }, { cause: error });
      throw error;
    }
  ...
  return {
    ...
    async release(holdRef) {
      ...
      const statements = [
        ...
        prepared(
          db
            .updateTable(WALLET_HOLDS_TABLE)
    ...
    async transactions(userId, currency, limit) {
      const rows = await db
        .selectFrom(WALLET_TRANSACTIONS_TABLE)
        .selectAll()
        .where("userId", "=", userId)
        .where("currency", "=", currency)
        .orderBy("id", "desc")
        .limit(limit)
        .execute();
      return rows.map((row) => WalletTransaction.parse(row));
    },
  };
}
...
async function requireOpenHold(
  db: ReturnType<typeof walletDatabase>,
  ref: string,
): Promise<{ userId: string; currency: string; amount: number }> {
  const row = await db
    .selectFrom(WALLET_HOLDS_TABLE)
    .select(["userId", "currency", "amount", "status"])
    .where("ref", "=", ref)
    .executeTakeFirst();
  if (!row) throw new WalletHoldNotFoundError({ detail: `No hold with ref ${ref}.` });
  if (row.status !== "open") throw new WalletHoldNotOpenError({ detail: `Hold ${ref} is ${row.status}.` });
