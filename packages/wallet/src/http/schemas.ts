...
/**
 * The HTTP boundary shapes for the wallet routes. Everything a client can send is declared here and
 ...
 * `wallet/currency_not_found` — a 404, because an unconfigured `doubloons` is a missing resource, not a
...
export const CurrencyParam = z
  ...
  .describe("The `:currency` path segment every wallet route carries.");
