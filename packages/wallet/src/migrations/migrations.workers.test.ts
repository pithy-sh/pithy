...
import { wallet_0001_ledger } from "./0001_ledger";

const db = () => createDatabase(env.DB, {}) as unknown as Kysely<unknown>;

async function catalog(): Promise<string[]> {
  const { results } = await env.DB.prepare(
    "SELECT name FROM sqlite_master WHERE name LIKE 'pithy_wallet_%' ORDER BY name",
  ).all<{ name: string }>();
...
describe("wallet_0001_ledger", () => {
  ...
  test("the accounts CHECK constraint rejects a negative balance at the DB level", async () => {
    ...
    await expect(
      env.DB.prepare(
        "INSERT INTO pithy_wallet_accounts (user_id, currency, balance, held, created_at, updated_at) VALUES ('x','chips',-1,0,0,0)",
      ).run(),
    ).rejects.toThrow(/CHECK constraint failed/i);
    // held may not exceed balance either.
    await expect(
      env.DB.prepare(
        "INSERT INTO pithy_wallet_accounts (user_id, currency, balance, held, created_at, updated_at) VALUES ('y','chips',10,20,0,0)",
      ).run(),
    ).rejects.toThrow(/CHECK constraint failed/i);
  });
  ...
  test("down is the exact inverse and up is re-runnable after it", async () => {
    await wallet_0001_ledger.up(db());
    await wallet_0001_ledger.down?.(db());
    expect(await catalog()).toEqual([]);
    await wallet_0001_ledger.up(db());
    expect(await catalog()).toContain("pithy_wallet_accounts");
  });
});
