...
/**
 * The wallet ledger: accounts, the append-only transaction log, and holds.
 *
 ...
 */
export const wallet_0001_ledger: Migration = {
  up: async (db: Kysely<unknown>): Promise<void> => {
    await db.schema
      .createTable("pithyWalletAccounts")
      ...
      .addUniqueConstraint("pithyWalletAccountsOwnerIdx", ["userId", "currency"])
      // The overdraft guard: never negative, never over-reserved. SQLite aborts any statement that breaks it.
      .addCheckConstraint("pithyWalletAccountsSolvent", sql`balance >= 0 AND held >= 0 AND held <= balance`)
      .execute();

    await db.schema
      .createTable("pithyWalletTransactions")
      .addColumn("id", "integer", (c) => c.primaryKey())
    ...
    await db.schema
      .createIndex("pithyWalletTransactionsOwnerIdx")
      .on("pithyWalletTransactions")
      .columns(["userId", "currency", "id"])
      .execute();

    await db.schema
      .createTable("pithyWalletHolds")
      .addColumn("id", "integer", (c) => c.primaryKey())
      .addColumn("ref", "text", (c) => c.notNull().unique())
      .addColumn("userId", "text", (c) => c.notNull())
      .addColumn("currency", "text", (c) => c.notNull())
  ...
  down: async (db: Kysely<unknown>): Promise<void> => {
    await db.schema.dropTable("pithyWalletHolds").execute();
    await db.schema.dropIndex("pithyWalletTransactionsOwnerIdx").execute();
    await db.schema.dropTable("pithyWalletTransactions").execute();
    await db.schema.dropTable("pithyWalletAccounts").execute();
  },
};
