...
/**
 * One player's balance in one currency — the row in `pithy_wallet_accounts`, keyed `(userId, currency)`.
 *
...
export const WalletAccount = z
  ...
  .describe("One player's balance in one currency — the row in `pithy_wallet_accounts`.");
export type WalletAccount = z.output<typeof WalletAccount>;
export type WalletAccountRow = z.input<typeof WalletAccount>;
