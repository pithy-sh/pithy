...
import { wallet } from "../capability";
import { WALLET_ACCOUNTS_TABLE } from "../data/tables";
import { walletExampleSeed } from "./example";

const currencies = [{ code: "coins", name: "Coins" }];

describe("walletExampleSeed", () => {
  it("is flagged as an example", () => {
    expect(walletExampleSeed.example).toBe(true);
  });

  it("never lists production — an example fixture is dev/staging only", () => {
    expect(walletExampleSeed.environments).not.toContain("production");
    expect(walletExampleSeed.environments).toEqual(["dev", "staging"]);
  });

  it("seeds a demo balance for each canonical user into the app database", () => {
    expect(walletExampleSeed.d1).toHaveLength(1);
    const [group] = walletExampleSeed.d1 ?? [];
    expect(group?.database).toBe("app");
    expect(group?.table).toBe(WALLET_ACCOUNTS_TABLE);
...
describe("wallet() with seed.includeExamples", () => {
  it("composes the example set only when includeExamples is on", () => {
    const capability = wallet({ currencies });
    ...
    expect(withExamples.sets[0]?.key).toContain("wallet");
  });
