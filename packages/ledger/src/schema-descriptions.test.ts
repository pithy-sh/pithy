...
// Mirrors core's meta-test. The ledger and routes import `@cloudflare/workers-types` type-only (erased), so
// they load fine in node.
...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 21 modules, 24 schemas and 64 fields on
    ...
    expect(walk.fields).toBeGreaterThanOrEqual(60);
  });
