...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 40 modules, 23 schemas and 72 fields on
    ...
    expect(walk.modules).toBeGreaterThanOrEqual(38);
    expect(walk.schemas).toBeGreaterThanOrEqual(21);
    expect(walk.fields).toBeGreaterThanOrEqual(68);
  });
