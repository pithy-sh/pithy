...
- **Wallet settlement.** A model's `apply`/`resolve` are pure — they cannot touch a database — so a bet *declares* wallet effects (`hold` a stake, `capture` a loss, `credit` a win) that the DO settles through `@pithy-sh/wallet`'s ledger. A hold a player cannot cover rejects the action; a deterministic model re-emits the same effect refs, so a replay pays once. Install `@pithy-sh/wallet` for any wagering game.
