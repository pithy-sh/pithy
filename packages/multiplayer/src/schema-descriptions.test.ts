...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 27 modules, 44 schemas and 98 fields on
    ...
    expect(walk.schemas).toBeGreaterThanOrEqual(41);
    expect(walk.fields).toBeGreaterThanOrEqual(93);
  });
