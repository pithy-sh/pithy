...
export interface PackageAdvice {
  /** The command that clears the line, or `null` when no `pithy` command moves this package. */
  command: string | null;
 ...
 *
 * `specs` is every declaration of that one package. Nothing declared means nothing for `--packages` to
 * move: the copy is transitive, and the package that pulls it in is the one to move. An unmanaged spec is
 * one this command leaves alone, so naming it would be the recommendation #634 was filed about. Otherwise
 * the question is whether the default move reaches `latest` — which, since `latest` is the ceiling of the
 * candidates, is the question whether every range admits it.
 */
export function packageCommand(specs: readonly DeclaredSpec[], latest: string): PackageAdvice {
  if (specs.length === 0) return { command: null, declaredAs: null };
  const unmanaged = specs.find((spec) => parseRange(spec.spec) === null);
  if (unmanaged) return { command: null, declaredAs: unmanaged.spec };
  const reached = specs.every((spec) => {
    const range = parseRange(spec.spec);
    return range !== null && admits(range, latest);
  });
  return { command: reached ? PACKAGES_COMMAND : PACKAGES_LATEST_COMMAND, declaredAs: null };
}
