...
const RECORD: Record<string, string> = {
  ...
  "packages/ui-react/templates/src":
    "`ci/catalogCoverage.test.ts` reads the `satisfies MessageCatalog` block baked into every screen — the kit's English for everything a template renders, and the only copy of it, because a template is copied into the adopter's repository rather than imported from a package (#441). It is named rather than left to `packages` above, because the read is not about a manifest or a declared constant: a screen gaining one line of copy is what makes a Spanish reader meet English, and that edit must plan the suite that says so.",
  ...
  "scripts/cleanRoom.ts":
    "`@pithy-sh/release`'s `cleanRoom.test.ts` reads the entry point CI actually names, not only the module behind it. Three of its assertions are about one line each — the isolated `BUN_INSTALL_CACHE_DIR`, the per-step timeout and the stall-versus-failure report — and a unit test of a constant cannot say the entry point uses it. That mattered: a `bun add` deadlock held a release job for 1h42m and reported nothing, because the step had no bound and the clean room was sharing the machine's install cache. Editing the script must re-run the suite that holds it to those three.",
  "scripts/localRelease.ts":
    "`@pithy-sh/release`'s `localRelease.test.ts` reads the entry point the operator actually runs, for the reason the line above gives and one more: what it asserts is an *order*, and an order is not a constant a unit test can hold. `release:local` published twenty-two packages of stale compiled output because the build step was never in it — while this module's header, the script's header and `docs/RELEASING.md` all said the sequence was the workflow's. Three claims, no gate. So the four `run(\"bun\", ...)` calls are read in order and held to build-after-version, build-before-publish, with the pack check between. Editing the script must re-run the suite that says so.",
