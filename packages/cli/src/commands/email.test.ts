...
describe("pithy email", () => {
  ...
  // #591: a teardown walked every declared environment, production's email worker included.
  ...
  test("deprovision with no --env refuses and lists the environments it could act on, before any credential", async () => {
    ...
    const error = await failure("deprovision", { json: true, suppression: true });
  ...
  test("deprovision naming an undeclared environment refuses the same way", async () => {
    root.config = { name: "acme" };
    const error = await failure("deprovision", { json: true, suppression: true, env: "live" });
  ...
  test("deprovision answers --env production with prod", async () => {
    root.config = { name: "acme" };
    const error = await failure("deprovision", { json: true, suppression: true, env: "production" });
