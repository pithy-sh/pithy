...
function loadCloudflareCreds(projectDir: string): {
  ...
  if (!storeId) {
    throw new ValidationError({
      ...
      action: "Set SECRETS_STORE_ID in .dev.vars (the sweep worker decrypts its credentials from it).",
    });
  }
...
const provision = defineCommand({
  ...
  args: {
    ...
    "r2-access-key-id": {
      type: "string",
      description:
        "R2 S3 access key id the Worker presigns uploads and downloads with. Create the pair under R2 → Manage API tokens. Falls back to R2_CREDENTIALS in .dev.vars.",
    },
