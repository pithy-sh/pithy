...
/**
 * The project name is resolved at this command edge, and every storage name leads with it:
 * `<project>-<env>-storage` for the bucket, `<project>-<env>-storage` for the sweep worker. Both
 * subcommands must therefore refuse a project that cannot name itself — `deprovision` most of all, since
 * it finds what to delete by recomputing the name provisioning used. A guessed name (the alphabetically
 * first worker, the directory basename) would compute names matching nothing, delete nothing, and exit 0.
 */
...
function subcommand(name: string): CommandDef {
  const entry = (storage.subCommands as Record<string, CommandDef>)[name];
