...
/**
 * The project name is resolved at this command edge, and every media name leads with it — the R2 bucket,
 * the MEDIA KV namespace, the derivative worker. Both subcommands refuse a project that cannot name
 * itself, `deprovision` most of all: it finds what to delete by recomputing the name provisioning used,
 * so a guessed name would match nothing, delete nothing, and exit 0.
 */
...
function subcommand(name: string): CommandDef {
  const entry = (media.subCommands as Record<string, CommandDef>)[name];
