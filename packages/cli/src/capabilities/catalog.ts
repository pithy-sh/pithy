...
export const CATALOG: readonly CatalogEntry[] = [
  ...
  {
    name: "organization",
    package: "@pithy-sh/organization",
    whenToEnable:
      "Tenancy, out of the box: organizations, memberships, roles you define with the powers they hold, invitations bound to an address, ownership that moves only when somebody accepts it, and a chooser for people who belong to more than one. Every multi-tenant project on this kit writes this model, and this is the one the Pithy dashboard itself runs on. You declare the roles, because a coaching academy's `coach` and `student` are parallel where a dashboard's `owner`, `admin` and `member` nest, and a capability that assumed either shape would refuse the other. The kit owns five power names because it ships the routes that gate on them; yours sit beside them under your own vocabulary, and your handlers import them as typed values rather than reading strings out of a config file. What a membership is worth is what makes the refusals load-bearing: a caller naming an organization they do not belong to gets a 404 byte-identical to the one for an organization that does not exist, because a distinguishable refusal is an existence oracle and iterating it produces your customer list. A role is decoded off the row and refused when the catalog does not know it, never asserted against a matrix that would deny everything today and allow it after one refactor. Nothing about a role rides on the session — only the id of the chosen organization — so removing a membership row is the whole of revocation and takes effect on the next request with no sign-out. The acting membership lands on its own context variable rather than on the auth one, because 'signed in' and 'a member of this organization' are two conditions and merging them makes them one. Composing this alongside Better Auth's organization() plugin is refused at boot: two membership models in one Worker is two answers to whether somebody may act.",
  },
  {
    name: "controlplane",
