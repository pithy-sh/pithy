...
describe("scaffoldWorker", () => {
  ...
  test("stamps the worker's files, including its own pithy.config.ts", async () => {
    const { dir: workerDir } = await scaffoldWorker({ projectDir: dir, name: "web" });
  ...
  test("a hyphenated worker gets a legal migration namespace, keeping its kebab-case directory", async () => {
    ...
    const { dir: workerDir } = await scaffoldWorker({ projectDir: dir, name: "admin-api" });
    ...
    // The Worker still deploys and installs under its real, kebab-case name.
    const wrangler = parse(await readFile(join(workerDir, "wrangler.jsonc"), "utf8")) as unknown as { name: string };
    expect(wrangler.name).toBe("admin-api");
  });
  ...
  test("rejects a non-kebab-case name", async () => {
    await expect(scaffoldWorker({ projectDir: dir, name: "Web_App" })).rejects.toThrow(PithyError);
  });
  ...
  test("refuses to overwrite a non-empty apps/<name>", async () => {
    ...
    await expect(scaffoldWorker({ projectDir: dir, name: "web" })).rejects.toThrow(PithyError);
  });
});
