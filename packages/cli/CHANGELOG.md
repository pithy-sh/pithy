...
- [#657](https://github.com/pithy-sh/pithy/pull/657) [`2fa6d65`](https://github.com/pithy-sh/pithy/commit/2fa6d659c87298db4de20afe0e5abf08c081b784) Thanks [@kingmesal](https://github.com/kingmesal)! - A feature deployment signs people in on Cloudflare's always-pass test keys, with nothing to provision and no adopter edit.
...
- [#655](https://github.com/pithy-sh/pithy/pull/655) [`6e466d7`](https://github.com/pithy-sh/pithy/commit/6e466d78afb06d121ff6fad3c2dec984ae9a171b) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy docs connect` wires the Pithy documentation server into your AI coding agent. Ten clients, three config formats, one command.
...
- [#649](https://github.com/pithy-sh/pithy/pull/649) [`aa8b18a`](https://github.com/pithy-sh/pithy/commit/aa8b18a99f9cb17486d34880e83c9b874f6c202d) Thanks [@kingmesal](https://github.com/kingmesal)! - Scheduled master-key rotation now proves each write landed before anything depends on it, so a rotation can no longer leave an environment's stored secrets undecryptable.
...
- [#653](https://github.com/pithy-sh/pithy/pull/653) [`b11d67b`](https://github.com/pithy-sh/pithy/commit/b11d67b4fe97ef0808ee031fcff7f92608370a4c) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy token mint ci-system` now scopes the CI token to the zones the project's declared domains sit in, so a CI deploy of a custom domain works.
  ...
  **`pithy token rotate` is the re-scope, and the tooling now says so.** A token that predates a permission — the zone route grant above, or a capability composed since — is replaced by rotate: a new token with the profile's current policies, stored, then the old one deleted. `pithy token mint` rolls the value on the same token and leaves its policies exactly as they were. `pithy token list` and the docs name rotate; naming mint would be a command that changes nothing.
  
  `ensureManagerToken` is unaffected: a mint that never touches policies is the behavior the secrets manager's runtime credential already wanted, so it needs no path of its own.
  
  A mint over a `disabled` or `expired` token now says so, in the human output and in `--json`, instead of writing a real value for a dead credential and printing `Done.`
  ...
  A grant missing because a standing `tokens.overrides` removes it reads as its own state, and the notice names the override instead of a command — rotate honors the override too, so printing it would be a loop that never ends. A `disabled` or `expired` token reads as its own state as well: Cloudflare refuses it whatever it is scoped to, and calling that `scoped` tells an operator their next deploy will work.
...
- [#654](https://github.com/pithy-sh/pithy/pull/654) [`d2bb2e2`](https://github.com/pithy-sh/pithy/commit/d2bb2e2885d5e7d9f7b3bf8117951bfeaf333f9a) Thanks [@kingmesal](https://github.com/kingmesal)! - A feature deployment now binds the Workflows and Durable Objects the app declares, so it serves a request instead of answering 500.
...
- [#644](https://github.com/pithy-sh/pithy/pull/644) [`cca38dc`](https://github.com/pithy-sh/pithy/commit/cca38dc6f86fc734b1267689b2ef98654ee674f7) Thanks [@kingmesal](https://github.com/kingmesal)! - A feature deployment has an address. A feature Worker answers on `https://<script>.<subdomain>.workers.dev`, and `pithy provision --feature` now derives that from the Worker's script name and the account's `workers.dev` subdomain, through the same resolver staging and prod use, and stamps it as the feature stanza's `vars.BASE_URL`. `originFor` reads that stamp inside a feature deployment, so the scaffolded `PUBLIC_ORIGIN` — and with it auth's base URL, email's links and payments' return URLs — is the feature's own origin instead of `http://localhost`. Only a `workers.dev` https origin is read, so a `BASE_URL` a feature inherits from the top level is never taken for its own. A declared environment never falls back to `workers.dev`. `pithy dashboard connect --env feature` registers the same address.
...
- [#604](https://github.com/pithy-sh/pithy/pull/604) [`fc8e142`](https://github.com/pithy-sh/pithy/commit/fc8e142e382e3257e2da800869d958ec20c360d1) Thanks [@kingmesal](https://github.com/kingmesal)! - A Secrets Store binding is its key in SCREAMING_SNAKE_CASE.
  
  `pithy secrets provision` wrote `"binding": "email-link-signing-key"` into `wrangler.jsonc`. A binding is an environment name, and environment names are capitals. The kit read and wrote a `cf-secrets-store` secret's registry key as its binding, verbatim, which went unnoticed while every such key happened to be in capitals — until [#596](https://github.com/pithy-sh/pithy/issues/596) moved the kebab-case link-signing key into the store.
...
- Updated dependencies [[`fc8e142`](https://github.com/pithy-sh/pithy/commit/fc8e142e382e3257e2da800869d958ec20c360d1)]:
  - @pithy-sh/secrets@0.2.5
  - @pithy-sh/email@0.3.5
...
- [#599](https://github.com/pithy-sh/pithy/pull/599) [`ead1740`](https://github.com/pithy-sh/pithy/commit/ead17401ae8ad75366e86658604460d0aae14816) Thanks [@kingmesal](https://github.com/kingmesal)! - `vector`, `storage`, `media`, `payments` and `support` provision the capability the `--worker` target composes.
...
- [#601](https://github.com/pithy-sh/pithy/pull/601) [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f) Thanks [@kingmesal](https://github.com/kingmesal)! - The narration gate follows a relative `import()` written as a template literal, and refuses one built at runtime.
...
- [#601](https://github.com/pithy-sh/pithy/pull/601) [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy add`, `pithy remove`, `pithy worker add` and `pithy feature create` say when they are installing.
...
- [#599](https://github.com/pithy-sh/pithy/pull/599) [`ead1740`](https://github.com/pithy-sh/pithy/commit/ead17401ae8ad75366e86658604460d0aae14816) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy upgrade` no longer overwrites an option a registration supplies as a shorthand or through a spread.
...
- Updated dependencies [[`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`db6674a`](https://github.com/pithy-sh/pithy/commit/db6674a775fc14c4e20410352bdd90c1c54abed8), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`db6674a`](https://github.com/pithy-sh/pithy/commit/db6674a775fc14c4e20410352bdd90c1c54abed8), [`db6674a`](https://github.com/pithy-sh/pithy/commit/db6674a775fc14c4e20410352bdd90c1c54abed8), [`db6674a`](https://github.com/pithy-sh/pithy/commit/db6674a775fc14c4e20410352bdd90c1c54abed8), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`d7a7168`](https://github.com/pithy-sh/pithy/commit/d7a7168e2ce7d2769d220e39d55e095a4477a31f), [`ead1740`](https://github.com/pithy-sh/pithy/commit/ead17401ae8ad75366e86658604460d0aae14816), [`ead1740`](https://github.com/pithy-sh/pithy/commit/ead17401ae8ad75366e86658604460d0aae14816), [`32186ff`](https://github.com/pithy-sh/pithy/commit/32186ff9efe385665496f1ecafe315f4e248ae3a)]:
  - @pithy-sh/core@0.7.0
  - @pithy-sh/secrets@0.2.4
  - @pithy-sh/email@0.3.4
  - @pithy-sh/cloudflare@0.3.0
  - @pithy-sh/turnstile@0.3.0
...
- Updated dependencies [[`7d9a7c4`](https://github.com/pithy-sh/pithy/commit/7d9a7c48e26dffd9d1062af0b743049b967e78e1)]:
  - @pithy-sh/cloudflare@0.2.3
  - @pithy-sh/core@0.6.1
  - @pithy-sh/email@0.3.3
  - @pithy-sh/secrets@0.2.3
  - @pithy-sh/turnstile@0.2.3
...
- [#556](https://github.com/pithy-sh/pithy/pull/556) [`65d9ba0`](https://github.com/pithy-sh/pithy/commit/65d9ba01ebb3790f744306b3a7e6042212d6c369) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy dev` now authenticates every worker as the account your project claims.
...
- Updated dependencies [[`65d9ba0`](https://github.com/pithy-sh/pithy/commit/65d9ba01ebb3790f744306b3a7e6042212d6c369)]:
  - @pithy-sh/email@0.3.0
...
- [#549](https://github.com/pithy-sh/pithy/pull/549) [`ba0f517`](https://github.com/pithy-sh/pithy/commit/ba0f51765d2246f725c3ded6a3f0202443f24713) Thanks [@kingmesal](https://github.com/kingmesal)! - Keeping a worker out of your local dev set is your decision, not the project's.
...
- [#543](https://github.com/pithy-sh/pithy/pull/543) [`729b46f`](https://github.com/pithy-sh/pithy/commit/729b46f366dadf190da11b67fb9063b20fd42e26) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy dev --list` shows the Workers a run would start, hosts included. `--app` starts only the ones you name. Every Worker autostarts now — set `dev.autostart: false` for one that should not.

- [#544](https://github.com/pithy-sh/pithy/pull/544) [`3e55d2f`](https://github.com/pithy-sh/pithy/commit/3e55d2f467527c5f329b47e81c3d150c701feef9) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy deploy` deploys everything the project deploys, and re-uploads a kit Worker only when it changed.
...
- [#544](https://github.com/pithy-sh/pithy/pull/544) [`3e55d2f`](https://github.com/pithy-sh/pithy/commit/3e55d2f467527c5f329b47e81c3d150c701feef9) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy doctor` reports two Workers holding different versions of one capability.
...
- [#546](https://github.com/pithy-sh/pithy/pull/546) [`caab3fa`](https://github.com/pithy-sh/pithy/commit/caab3fab1c08b7f4b8f515c03d7d58e08e4f7bae) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy secrets ls` and `pithy doctor` stop asking for credentials the configuration has turned off.
...
- Updated dependencies [[`caab3fa`](https://github.com/pithy-sh/pithy/commit/caab3fab1c08b7f4b8f515c03d7d58e08e4f7bae), [`3e55d2f`](https://github.com/pithy-sh/pithy/commit/3e55d2f467527c5f329b47e81c3d150c701feef9), [`3e55d2f`](https://github.com/pithy-sh/pithy/commit/3e55d2f467527c5f329b47e81c3d150c701feef9), [`3e55d2f`](https://github.com/pithy-sh/pithy/commit/3e55d2f467527c5f329b47e81c3d150c701feef9)]:
  - @pithy-sh/core@0.4.0
  - @pithy-sh/cloudflare@0.2.0
  - @pithy-sh/email@0.2.0
  - @pithy-sh/secrets@0.2.0
  - @pithy-sh/turnstile@0.2.0

## 0.2.3

### Patch Changes

- [#540](https://github.com/pithy-sh/pithy/pull/540) [`231b726`](https://github.com/pithy-sh/pithy/commit/231b72603b284c139e00eab92a52af34a7e86bd3) Thanks [@kingmesal](https://github.com/kingmesal)! - A globally installed `pithy` resolves capabilities against the project, not against itself.
...
- [#540](https://github.com/pithy-sh/pithy/pull/540) [`231b726`](https://github.com/pithy-sh/pithy/commit/231b72603b284c139e00eab92a52af34a7e86bd3) Thanks [@kingmesal](https://github.com/kingmesal)! - A `json` secret has two stored forms, `pithy turnstile provision` wrote one of them into the other's destination, and a value its own registry refuses is now refused at the write.
...
- [#540](https://github.com/pithy-sh/pithy/pull/540) [`231b726`](https://github.com/pithy-sh/pithy/commit/231b72603b284c139e00eab92a52af34a7e86bd3) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy doctor` reports a capability it cannot reach, and stops calling Cloudflare reachable for products it never asked about.
...
- Updated dependencies [[`231b726`](https://github.com/pithy-sh/pithy/commit/231b72603b284c139e00eab92a52af34a7e86bd3), [`231b726`](https://github.com/pithy-sh/pithy/commit/231b72603b284c139e00eab92a52af34a7e86bd3)]:
  - @pithy-sh/cloudflare@0.1.7
  - @pithy-sh/core@0.3.1
  - @pithy-sh/turnstile@0.1.7
  - @pithy-sh/secrets@0.1.7

## 0.2.2

### Patch Changes

- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - Capability provisioning skips an environment that has no app database, and says which and why.
...
- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - A binding says whether its resource belongs to the project or to the environment.
  ...
  **The divergent half has its own remedy now, because it is a different job.** Two stanzas that carry the right name and two different `database_id`s have nothing stale by name, so "export each old database, then execute it against `<project>-global-email-suppressions`" named one string as both source and destination — a database exporting into itself, and `wrangler d1 export` addresses a database by name or binding and never by id anyway. What is true there is that two resources are open, one survives, and `pithy provision` picks it by resolving the *name* on your account rather than by reading the ids — so the decision is yours, the ids in the report are what it is made from, and the losing resource's rows move first. The run lines are enumerated per environment from `ids[].at[].env`, which the entry carried all along while the remedy printed a `<env>` placeholder nobody could paste. And the copy is addressed differently: two databases answering to one name is exactly the state that name cannot resolve, so the branch sends you to a subsection that reaches each one through `-e <env>` and its binding, which is the only form that does.
  ...
  `pithy provision` also names any installed `pithy.manifest.json` it could not read — the `faults` half of the scan it had been discarding ([#184](https://github.com/pithy-sh/pithy/issues/184)). The bindings still come from the composed capability, so the resources are still created; what a broken manifest costs is the `scope` and the `resource` that decide what they are **called**, so a project-global database gets created per environment and the run reports success. It is in the plan printed before the confirmation, in the summary, and in `--json` as `manifestFaults`.
  
  `MEDIA_BUCKET` and `STORAGE_BUCKET` had the same split without the `global` half — `<project>-<env>-media-bucket` against `<project>-<env>-media`. [#519](https://github.com/pithy-sh/pithy/issues/519) closes both in this release, and the gate has no exemption rows left.

- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy provision` honors a declined binding, and says what it left out.
  
  `declinedBindings` was read by the reader and ignored by the writer: `pithy doctor` reported the declaration correctly while `provision` created the resource and wrote the binding back into the file the adopter had removed it from. [#440](https://github.com/pithy-sh/pithy/issues/440) fixed this for `pithy upgrade` and provision never got the same treatment.
...
- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy provision` says what it will do, then says what it is doing.
...
- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy secrets create` asks a `json` secret one field at a time, instead of one blind blob.
...
- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - A secret write knows where the value goes, and `pithy secrets rm` performs the revocation it reports.
...
- [#531](https://github.com/pithy-sh/pithy/pull/531) [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5) Thanks [@kingmesal](https://github.com/kingmesal)! - One namer for one bucket: `MEDIA_BUCKET` and `STORAGE_BUCKET` are composed once.
  
  [#513](https://github.com/pithy-sh/pithy/issues/513) gave `BindingSpec` a `resource` field — the `<thing>` segment where it is not the kebabbed binding name — and pointed every writer at it, and left these two out because they are the `resource` half without the `scope` half. So `pithy add` and `pithy provision` went on composing `<project>-<env>-media-bucket` from the binding name while `mediaBucketName` composed `<project>-<env>-media`. Two names for one bucket, which is exactly the property [#513](https://github.com/pithy-sh/pithy/issues/513) exists to remove. `storage` had it identically.
...
- Updated dependencies [[`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5), [`a24ebff`](https://github.com/pithy-sh/pithy/commit/a24ebff4bbe18f5571344dd3f79113a809494fc5)]:
  - @pithy-sh/email@0.1.7
  - @pithy-sh/core@0.3.0
...
- [#508](https://github.com/pithy-sh/pithy/pull/508) [`a6b4961`](https://github.com/pithy-sh/pithy/commit/a6b4961bcaa890437ff0707df064823485633d38) Thanks [@kingmesal](https://github.com/kingmesal)! - `pithy doctor` and `pithy upgrade` see the capabilities a Worker composes, not the ones the root happens to install.
...
- [#494](https://github.com/pithy-sh/pithy/pull/494) [`b887113`](https://github.com/pithy-sh/pithy/commit/b887113ab50ba3f1cf80e68f9148319d0839b172) Thanks [@kingmesal](https://github.com/kingmesal)! - A command no longer loads a network client to print its own flag list.
...
- Updated dependencies [[`82bb9a0`](https://github.com/pithy-sh/pithy/commit/82bb9a0ce3a70ac0f66cc86d8b7bae64f9a3109e), [`509d921`](https://github.com/pithy-sh/pithy/commit/509d921580b6630d27a7534f29206e8e8a4d3678), [`1b3a116`](https://github.com/pithy-sh/pithy/commit/1b3a116e2f21d9c80fa1e494270205f7f8224c2c)]:
  - @pithy-sh/core@0.1.5
  - @pithy-sh/cloudflare@0.1.4
  - @pithy-sh/email@0.1.4
  - @pithy-sh/secrets@0.1.4
  - @pithy-sh/turnstile@0.1.4
  - @pithy-sh/ui-react@0.1.4
