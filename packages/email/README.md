...
**The list carries what a message was, not what it said.** `template`, `category`, `mode`, `bounceType` and `locale` name the kind of mail and nothing of its contents, so a page of them is a page of facts about your project rather than about people. The rendered `subject` is the other kind — it routinely names somebody's own order or their own document — so it stays on the detail route beside the whole address. That is what puts the locale tag on the list: "why is half this account getting English" is a question about a page of rows, and answering it one detail request at a time means a hundred audited disclosures to render one column. `null` there means what it means everywhere else — nobody chose, and it went out in the kit's English.
