...
// they load fine in node. `workflows/worker.ts` is the exception: it imports `cloudflare:workers` at
// runtime, which no node loader can resolve, so it is excluded rather than being allowed to fail the
// whole meta-test. Nothing is lost — every schema it uses is declared in `workflows/specs.ts`, which is
// in the glob.
...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 28 modules, 38 schemas and 74 fields on
    ...
    expect(walk.modules).toBeGreaterThanOrEqual(26);
    expect(walk.schemas).toBeGreaterThanOrEqual(36);
    expect(walk.fields).toBeGreaterThanOrEqual(70);
  });
