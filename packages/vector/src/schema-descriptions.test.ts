...
// Mirrors core's meta-test. Every module here imports `@cloudflare/workers-types` type-only (erased), so they
// load fine in node. A module that imports `cloudflare:workers` at runtime — a Workflow entrypoint — must be
// added to this glob as an exclusion, or the whole meta-test fails on an unrelated-looking resolution error.
...
describe("schema descriptions (CLAUDE.md §Zod: schemas are the docs)", () => {
  test("the sweep is looking at the package, not at nothing", () => {
    ...
    //
    // Near-exact, not a comfortable floor: measured at 29 modules, 24 schemas and 50 fields on
    ...
    expect(walk.modules).toBeGreaterThanOrEqual(27);
    expect(walk.schemas).toBeGreaterThanOrEqual(22);
    expect(walk.fields).toBeGreaterThanOrEqual(47);
  });
