...
**A static site gets the same quote path as one script tag.** `dist/paddle-prices.iife.js` — built by this package, published as `@pithy-sh/payments/paddle-prices.iife.js` — reads its account and its plan-to-price mapping off its own `<script data-paddle-*>` tag and writes each formatted total, and the tax sentence that makes it true, into the page's `[data-price-plan]` and `[data-price-note]` slots. The rail is named on the tag and in the artifact, never in the markup, so a pricing page's slots outlive the choice of provider. It exists because the marketing site and the dashboard each carried their own copy of that mechanism, and `#416` was the reviewed copy being wrong while the hand-written one was right, with nothing running both. `docs/paddle.md` §11 has the tag and the refusal rules.

**Prices in whole units are asked for, never assumed.** `wholeUnits: true` on `quotePlans`, or `data-paddle-whole-units="on"` on the tag, drops a fraction that is entirely zero — `$6.00` as `$6`, `<U+200F>٦٫٠٠٠ د.ك.<U+200F>` as `<U+200F>٦ د.ك.<U+200F>` — and nothing else, ever. It is decided from Paddle's minor-unit amount rather than by pattern-matching the rendered string, which is what makes it right in a three-decimal currency and in a script whose zero is not `0`. The two surfaces that tried it from the string alone wrote the same wrong regex (`#427`).
