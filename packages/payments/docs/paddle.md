...
| `adjustment.read` | telling a full refund from a partial one, and seeing whether one is already standing |
| `adjustment.write` | `POST /payments/subscription/refund`, if you offer refunds |
...
**`adjustment.write` is only needed if you offer self-service refunds.** `POST /payments/subscription/refund` raises one full refund adjustment per payment on the subscription. Paddle holds most live ones at `pending_approval` until somebody there reviews them, so the route reports a request rather than a payout — and a key without this permission refuses with the permission named in the detail, rather than failing somewhere a customer can see.
