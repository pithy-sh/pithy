...
describe("payments()", () => {
  ...
  test("contributes its routes, mounted under the configured basePath", () => {
    ...
    expect(paths).toEqual([
      ...
      "/billing/purchases",
      "/billing/restore",
      // The subscription lifecycle (#465). The read is one path and the four verbs are segments under it,
      // so a `/subscription` prefix is one surface rather than five unrelated routes that happen to rhyme.
      "/billing/subscription",
      "/billing/subscription/cancel",
      "/billing/subscription/change",
      "/billing/subscription/keep",
...
describe("payments().compose", () => {
  ...
  test("refuses a grants clause with no ledger composed at all", () => {
    ...
    expect(detail).toContain("coins_100");
    expect(() => compose(payments(GRANTING_CATALOG), [])).toThrow(
      'No ledger is composed in this Worker, and product "coins_100" credits a balance through one.',
    );
  });
  ...
  test("refuses a composed ledger too old to carry the surface a credit goes through", () => {
    ...
    expect(() => compose(payments(GRANTING_CATALOG), [old as Capability])).toThrow(
      "The composed ledger is too old for payments to credit a balance.",
    );
    expect(() =>
