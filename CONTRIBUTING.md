...
**The condition is the whole rule, so read it before copying the shape — and it has already flipped once.** All 22 packages were released at `0.1.0` on 2026-09-05, so their chains are history now and are append-only. What follows applies to a capability that has *not* shipped yet: a new package sits at `0.0.0` until its first release, and until then nothing anywhere has run its migrations. A migration chain buys exactly one thing: the ability to walk a database that already holds rows from an old shape to a new one. There is no such database, so a `0002` today is a step from a shape that never ran to a shape that never shipped — two `down`s, two test suites, and a history nobody will replay. Git already records when each column arrived, which is the only thing a chain actually preserves. Three capabilities had drifted into one anyway (#276).
...
`packages/cli/src/migrations/oneMigration.test.ts` enforces the rule for exactly the packages it still holds for — it filters on the version, so it went dormant for the 22 released at `0.1.0` and stays live for the next capability that lands at `0.0.0`. It states the invariant twice over: every authored migration is numbered `0001` (the number orders a migration *within its database*, so a `0002` is by definition a second step against one that already has a first), and a package authors no more migrations than the databases it declares a `<NAME>_MIGRATION_ORDER` for.
...
`*.workers.test.ts` projects state neither of those two, deliberately. workerd does not inherit the host environment — no token and no `HOME` — so there is nothing to blank and nothing to relocate. A test there sees Vite's `import.meta.env` shims, vitest's two pool ids, and whatever `bindings` its own config declares. Nothing else.

They state a third instead: `setupFiles: [WORKERS_ENV_SETUP]`, which is `vitest.workers.setup.ts` at the repository root. Inheritance is not the whole story — that last clause is, and a `bindings` entry writes into workerd's `process.env` by design. So the guard refuses any Cloudflare credential visible in there, whatever declared it, and the gate below refuses a workers config that reads `process.env` at all. Both, because a declaration reading the operator's shell carries nothing on a machine with no token exported, which is every CI runner: the runtime guard would pass it and the source check will not. The guard leaves its scan on `globalThis` and that same core test reads it back, because a setup file emptied to `export {};` satisfies every gate that only cites it.

The guard reads two environments, and a workers config states `compatibilityFlags: ["nodejs_compat"]` for the same reason. Without that flag a declared binding never reaches `process.env` — measured: drop the line, declare `bindings: { CLOUDFLARE_API_TOKEN: "leaked" }`, and the process scan returns `[]` while every test can still read the credential through `env` from `cloudflare:test`. So the guard reads that `env` too, and the gate holds every workers config to the flag. The source scan reaches further than the config as well: it covers every repository module a config imports, derived by walking the imports, because a helper one call away in `vitest.shared.ts` lands in a `bindings` entry just as directly.
...
**You need none of it to work on the kit, and a run says what it is missing.** Some live suites need a fixture a human makes once in a console — a Turnstile widget, a zone with Email Routing on it, an OAuth client. [`docs/FIXTURES.md`](docs/FIXTURES.md) is the whole list, and one helper resolves them: `fixtureReady("turnstile-widget")` is the boolean a `describe.skipIf` negates, and the run's `globalSetup` prints a line per fixture naming what is absent, what will therefore skip, and the section that creates it. **Absent means skip, never fail** — a checkout with no credentials runs green. Never read `process.env` for a fixture in a live suite: a second spelling is a fixture the report cannot see, and a suite it cannot explain.
...
One live test actually deploys Workers: the secrets provision → write/rotate/audit → teardown round trip (`secretsProvisioner.integration.test.ts`). It provisions both managers, writes a secret through the write Workflow (with an in-worker `audit` that decrypts and confirms the round trip — only a boolean leaves the worker, never the value), triggers the at-rest rotation Workflow and checks the stored `key_version` advances, then deletes everything. It runs under the reserved project `pithy-int-test`, so **every** name it touches — the manager Workers and their D1s, both Workflows, the Secrets Store entries, the CF API token it mints — is `pithy-int-test-…`, and teardown recomputes exactly those, never a real project's. It is still double-gated — it needs the creds above **and** the `live-deploy` fixture — because it really does deploy Workers, run Workflows, and write to the account's one Secrets Store. Having an account is not consent to deploy into it. `PITHY_LIVE_DEPLOY` is that switch, a word rather than a non-empty string: `1`/`true`/`yes`/`on` arms it, `0`/`false`/`no`/`off` declines, anything else is malformed and skips ([`docs/FIXTURES.md`](docs/FIXTURES.md#live-deploy)). Run it deliberately (`PITHY_LIVE_DEPLOY=1 bun run --filter @pithy-sh/cli test:integration`).
